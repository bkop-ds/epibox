...
epibox.loginObservable = async function () {
    ...
    if (epibox.parms.code) { // POST dance with code
        ...
        if (localStorage.epiBoxToken) { // pre-existing credentials found
            ...
            <p>
            <button onclick="epibox.setURL('https://account.box.com/api/oauth2/authorize?client_id=${epibox.oauth.client_id}&response_type=code&redirect_uri=https://observablehq.com/d/124de20ae820d0e2')" style="background-color:yellow">Login Box</button>
            </p>&nbsp;`
        }
    }
